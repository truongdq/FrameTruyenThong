...
namespace Ecotek.Common.SupperSocket
{
    ...
    {
        public uint address;  //< The 16-bit address field.
    ...
    public class IntelHex
    {
        ...
        List<IntelHexStructure> irecList = new List<IntelHexStructure>();   // internal structure that holds the record information.
        ...
        /// <returns>IntelHexStructure instance or null, if null then query Status class variable for the error.</returns>
        public List<IntelHexStructure> Read(StreamReader inStream)
        {
            string recordBuff;
            int dataCount, i;
            ...
            if (inStream == null)
            {
                return new List<IntelHexStructure>();
            }
        ...
        public string Print(bool verbose = false)
        {
            ...
            foreach (var irec in irecList)
            {
                for (int i = 0; i < irec.dataLen; i++)
                {
                    returnString += string.Format("{0:X02}", irec.data[i]);
                }
